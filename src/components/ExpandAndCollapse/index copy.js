...
import "./index.css";
import { Handle } from "reactflow/dist/esm";
  ...
  useEffect(() => {
    setNodes([
      ...initialNodes.map((item) => {
        return {
          id: item.id,
          type: item?.children?.length ? "input" : "output",//"card",//item?.children?.length ? "input" : "output",
    ...
    if (!findChildren.length) {
      const itemChildren = [
        ...data.data.children.map((item, i) => {
          return {
            id: item.id,
            type: item?.children?.length ? "default" : "output",//"card",//item?.children?.length ? "default" : "output",
            data: {
              label: item.name,
...
export default () => (
 ...
 function CardPlus({
    ...
    return (
      <div className="border-4 border-purple animate__animated animate__fadeInUp animate__faster bg-white flex justify-center   rounded-[10px] p-3 w-[250px] h-[150px] relative z-20">
        ...
        <div className="grid grid-cols-3 items-center gap-2 w-full">
          ...
          <div className="flex justify-between gap-3 text-[10px] w-full col-span-full">
            <div className="border-[3px] flex items-center bg-[#D2F5DC] text-green-600 border-green-400 p-2 h-7 rounded-full shrink-0">
              Revenue: $2500
            </div>
            <div className="border-[3px] flex items-center bg-[#D2F5DC] text-green-600 border-green-400 p-2 h-7 rounded-full shrink-0">
              Revenue: $2500
          ...
          >
            ...
            >
              <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M15.2888 3.25833C8.3982 3.25833 2.81226 8.84427 2.81226 15.7349C2.81226 22.6255 8.3982 28.2114 15.2888 28.2114C22.1794 28.2114 27.7654 22.6255 27.7654 15.7349C27.7654 8.84427 22.1794 3.25833 15.2888 3.25833ZM9.05053 15.7349C9.05053 15.0458 9.60913 14.4872 10.2982 14.4872H20.2794C20.9685 14.4872 21.5271 15.0458 21.5271 15.7349C21.5271 16.4239 20.9685 16.9825 20.2794 16.9825H10.2982C9.60913 16.9825 9.05053 16.4239 9.05053 15.7349Z"
                fill="#F46D22"
              />
            </svg>
          </button>
